...
import { Package, Calendar, MapPin, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function OrdersPage() {
  ...
  useEffect(() => {
    loadOrders();
  }, [currentUser, sortBy, sortOrder, currentPage]);

  const loadOrders = async () => {
    if (currentUser?.email) {
      try {
        setLoading(true);
        const sortParam = `${sortBy === 'date' ? 'orderDate' : sortBy},${sortOrder}`;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between text-sm text-gray-600">
        <span>{t('manageBooks.showing', { current: orders.length, total: orders.length })}</span>
            ...
            <CardHeader>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="text-right">
                  ...
                  <span className="text-3xl font-semibold text-primary">{order.price.toLocaleString()} {t('common.currency')}</span>
                  {order.usedBonuses && order.usedBonuses > 0 && (
                    <p className="text-xs text-green-600 mt-1">
                      {t('orders.bonusesUsed')}: -{order.usedBonuses} {t('common.currency')}
                    </p>
                  )}
                </div>
              </div>
